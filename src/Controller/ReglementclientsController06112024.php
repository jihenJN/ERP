...
class ReglementclientsController extends AppController
{
    ...
    public function getids()
    {
        ...
        (numero, date, utilisateur_id, compte_id, montanttotal, observation, situation, Nomberpiece, integre, name, montant, datefin, dateimp) 
        VALUES 
        ($numero, '$date', $utilisateur_id,$compte_id, $montanttotal, '$observation', '$situation', $Nomberpiece, $integre, $name, $montant, '$datefin', '$dateimp')");
    ...
    public function view($type = null, $id = null)
    {
        ...
        if ($cli != null) {
            ...
            $livraisons = $connection->execute("select * from bonlivraisons where (bonlivraisons.client_id=" . $cli . " and bonlivraisons.typebl=1 and bonlivraisons.totalttc > bonlivraisons.Montant_Regler) OR (bonlivraisons.id in" . $l . ");")->fetchAll('assoc');
    ...
    public function add($type = null, $client_id = null)
    {
        ...
        if ($client_id != null) {
            ...
            $factureavoirs = $this->fetchTable('Factureavoirs')->find('all')->where(['Factureavoirs.client_id =' . $client_id]);
        } else {
