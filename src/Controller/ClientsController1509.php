...
class ClientsController extends AppController
{
    ...
    public function view($id = null)
    {
        ...
        $this->set(compact('client', 'commercials', 'gouvernorats', 'exonerations', 'paiements', 'adressees', 'responsable', 'banquess', 'banques'));
    }
                ...
                if (isset($this->request->getData('data')['responsable']) && (!empty($this->request->getData('data')['responsable']))) {
                    foreach ($this->request->getData('data')['responsable'] as $i => $b) {
                        ...
                        if ($b['supresponsable'] != 1) {
                            ...
                            $this->fetchTable('Clientresponsables')->save($clientresponsable);
                        } else if ($b['supresponsable'] == 1 && $b['id']) {
                            $clientresponsable = $this->fetchTable('Clientresponsables')->get($b['id'], [
                                'contain' => []
                            ]);
    ...
    public function delete($id = null)
    {
        ...
        if ($this->Clients->delete($client)) {
            $this->Clientbanques->delete($clientbanques);
            $this->Adresselivraisonclients->delete($adresselivraisonclient);
            $this->Clientresponsables->delete($clientresponsables);
