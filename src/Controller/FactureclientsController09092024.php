...
class FactureclientsController extends AppController
{
    ...
    public function addf($type = null)
    {
        ...
        if ($this->request->is('post')) {
            ...
            if ($this->Factureclients->save($factureclient)) {
                ...
                if ($this->request->getData('Montant_Regler') != '0' || $this->request->getData('Montant_Regler') != 0) {
                    ...
                    $t['bonlivraison_id'] = $factureclient_id;
        ...
        $this->set(compact('escompte', 'paiements', 'mm', 'transporteurs', 'type', 'valeurs', 'caisses', 'banques', 'type', 'dates', 'mm', 'articles', 'factureclient', 'clients', 'depots', 'materieltransports', 'cartecarburants', 'chauffeurs', 'conffaieurs', 'factureclients', 'adresselivraisonclients'));
    }
    ...
    public function etatjournal()
    {
        ...
        if ($this->request->getQuery()) {
            ...
            //  ->where(['Clients.typeexoneration_id' => 2]);



            //   $exo = 'Clients.typeexoneration_id = 2';
            // $conddd = 'Factureclients.client_id = ' . $clientid;

            $factureclients = $this->fetchTable('Factureclients')->find('all', [
                'contain' => ['Clients'],
            ])
                // var_dump($factureclients->toarray());
                ->where([$cond1, $cond2]);
        }
        ...
        // foreach ($ligneachat as $f) {

        //     //debug($l);

        //     $tablignelachats['client'] = $clientid;
        //     $tablignelachats['date'] = $f['date'];
        //     $tablignelachats['numero'] = $f['numero'];

        //     $tablignelachats['totalht'] = $f['totalht'];
        //     $tablignelachats['totalremise'] = $f['totalremise'];
        //     $tablignelachats['totalfodec'] = $f['totalfodec'];
        //     $tablignelachats['totaltva'] = $f['totaltva'];
        //     $tablignelachats['totalttc'] = $f['totaltttc'];
        //     $tablignelachats['timbre'] = $f['timbre'];
        //     // debug($tablignelachats);

        //     $historiquearticles = $this->fetchTable('Historiquearticles')->newEmptyEntity();
        //     $historiquearticles = $this->Historiquearticles->patchEntity($historiquearticles, $tablignelachats);
    ...
    public function impetatjournal()
    {
    ...
    public function imprimelistefactureclient()
    {
...

    {
        ...
        foreach ($reglementclients as $f) {
            ...
            ])->where($pieceConditionsWithRegId);
    ...
    public function etatsoldeclient()
    {
        ...
        $unpaidInvoices = $this->Factureclients->find()
            ->contain(['Clients'])
            ->where([
                'Factureclients.Montant_Regler' => 0, $conde1, $conde2
            ])
            ->toArray();
