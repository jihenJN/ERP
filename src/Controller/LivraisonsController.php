...
class LivraisonsController extends AppController
{
    ...
    public function view($id = null)
    {
        ...
        $lignes = $this->Lignelivraisons->find()
        ...
        $this->set(compact('machines', 'services', 'type', 'livraison', 'lignes', 'count', 'articles', 'fournisseurs', 'commandes', 'adresselivraisonfournisseurs','depots', 'cartecarburants', 'materieltransports'));
    }
                    ...
                    if ($this->Livraisons->save($livraison)) {
                        ...
                        if (isset($this->request->getData('data')['tabligne3']) && (!empty($this->request->getData('data')['tabligne3']))) {
                            ...
                            foreach ($this->request->getData('data')['tabligne3'] as $i => $livraison) {
                                ///debug($livraison);
                                if ($livraison['sup'] != 1 && (!empty($livraison['article_idd']))) {

                                    $data['article_id'] = $livraison['article_idd'];
                                    ...
                                    if ($this->Lignelivraisons->save($lignelivraisons)) {
                                        $article = $this->fetchTable('Articles')->get($livraison['article_idd']);
                                        // debug($article);die;
    ...
    public function getquantitedes()
    {
        ...
        echo json_encode(array("donnearticle" => $donnearticle, "success" => true));
        die;
    }
    ...
    public function edit($id = null)
    {
                        ...
                        if ($livraison['sup'] != 1 &&  (!empty($livraison['article_idd']))) {
                            ...
                            if ($commande->id) {
                                ...
                                foreach ($lignecommande as $lignecommande) {
                                    if ($livraison['article_idd'] == $lignecommande['article_id']) {
        ...
        $this->set(compact('machines', 'services', 'conffaieurs', 'chauffeurs', 'livraison', 'lignes', 'count', 'articles', 'fournisseurs', 'commandes', 'adresselivraisonfournisseurs','depots', 'cartecarburants', 'materieltransports', 'type'));
    }
    ...
    public function delete($id = null)
    {
        ...
        $livraison = $this->Livraisons->get($id);
        $type = $livraison->typelivraison;
        $lignecommandes = null; // Initialisation de $lignecommandes à null par défaut
