...
class ReglementsController extends AppController
{
    ...
    public function imprimengagement()
    {
        ...
        if ($this->request->getQuery()) {
            ...
            $echancefin = $this->request->getQuery('echancefin');
            ...
            if (!empty($echancefin)) {
                $cond8 = 'Piecereglements.echance <=  ' . "'" . $echancefin . "'";
            }
            ...
            $piecereglements = $piecereglementsTable->find()
                ->select([
                    ...
                    'reglement_id',
                    'compte_id',
                    'etatpiecereglement_id',
                    'num',
                    'echance',
                    'montant',
                    'month' => $piecereglementsTable->query()->func()->extract('MONTH', 'echance'),
                    'total' => $piecereglementsTable->query()->func()->count('*'),
                    'paiement_name' => 'Paiements.name', // Include paiement name
                    'compte_numero' => 'Comptes.numero' // Include compte numero
                ])
                ->where([$cond1, $cond3, $cond4, $condd, $cond6])->where([$cond1, $cond3, $cond4, $condd, $cond6,$cond8])->where('Piecereglements.paiement_id in (2,3)')
                 ->contain(['Paiements', 'Comptes']) // Include related tables
                 ->group(['month', 'paiement_id', 'reglement_id', 'compte_id', 'etatpiecereglement_id', 'num', 'echance', 'Piecereglements.montant'])
                 ->order(['Piecereglements.echance' => 'ASC']) 
                ->toArray();
            
            $organizedData = [];
            
            // Loop through each piecereglement
            foreach ($piecereglements as $dataItem) {
                // Get the month of the current piecereglement
                $currentMonth = '';
                $frenchMonths = [
                    'January' => 'Janvier',
                    'February' => 'Février',
                ...
                // Check if $dataItem->echance is not null before attempting to format it
                $currentMonth = '';
                if ($dataItem->echance !== null) {
                    // Format the echance date as 'F Y' (month name and year)
                    $currentMonth = $dataItem->echance->format('F Y');
                
                    // Extract the month name from the formatted date
                    $monthName = $dataItem->echance->format('F');
                    // Translate the English month name to French with uppercase first letter
                    $translatedMonth = $frenchMonths[$monthName] ?? $monthName;
                
                    // Extract the year from the formatted date
                    $year = $dataItem->echance->format('Y');
                
                    // Concatenate the translated month name and the year
                    $currentMonth = $translatedMonth . ' ' . $year;
                }
                
                // Initialize arrays for compte and paiement if not already initialized for the current month
                if (!isset($organizedData[$currentMonth][$dataItem->compte_id])) {
                    $organizedData[$currentMonth][$dataItem->compte_id] = [
                        'compte_numero' => $dataItem->compte_numero, // Add compte numero
                        'paiements' => []
                    ];
                }
                if (!isset($organizedData[$currentMonth][$dataItem->compte_id]['paiements'][$dataItem->paiement_id])) {
                    $organizedData[$currentMonth][$dataItem->compte_id]['paiements'][$dataItem->paiement_id] = [
                        'paiement_name' => $dataItem->paiement_name, // Add paiement name
                        'piecereglements' => []
                    ];
                }
            
                // Add the current piecereglement to the organized data
                $organizedData[$currentMonth][$dataItem->compte_id]['paiements'][$dataItem->paiement_id]['piecereglements'][] = $dataItem;
            }
            // Pass $organizedData to your view
         debug($organizedData);
        ...
        $this->set(compact('echancefin','organizedData','etatpiece_id', 'reglements', 'etatpiecereglement', 'agences', 'piecereglements', 'etats', 'combinedData', 'comptes', 'modes', 'fournisseurs', 'echance', 'modeid', 'compteid', 'modes', 'datereg', 'fournisseurid'));
    }
    public function indexeng()
    {
        ...
        if ($this->request->getQuery()) {
            ...
            $piecereglements = $this->Piecereglements->find('all')
                 ->contain(['Reglements'])
                ->where([$cond1, $cond3, $cond4, $condd, $cond6,$cond8,$cond2])->where('Piecereglements.paiement_id in (2,3)')
                ->order(['Piecereglements.echance' => 'ASC']) 
                ->toArray(); // Fetch results as an array
        ...
        $this->set(compact('echancefin','datefin','etatpiece_id', 'reglements', 'etatpiecereglement', 'agences', 'piecereglements', 'etats', 'combinedData', 'comptes', 'modes', 'fournisseurs', 'echance', 'modeid', 'compteid', 'modes', 'datereg', 'fournisseurid'));
    }
    /**
     * Index method
     *
    ...
    public function index()
    {
        ...
        $fournisseur_id = $this->request->getQuery('fournisseur_id');
        $pointdevente_id = $this->request->getQuery('pointdevente_id');
        $numerofac = $this->request->getQuery('numero');
        // debug($numerofac);
        if ($fournisseur_id != '') {
            $cond[] = ["Reglements.fournisseur_id" => $fournisseur_id];
        }
        if ($pointdevente_id != '') {
            $cond[] = ["Reglements.pointdevente_id like '%" . $pointdevente_id . "%'"];
        }
        if ($datedebut != '') {
            $cond[] = ['Reglements.Date >=' => $datedebut];
        }
        if ($datefin != '') {
            $cond[] = ['Reglements.Date <=' => $datefin];
        }
        if ($numerofac != '') {
            $condd = "factures.numero like '%" . $numerofac . "%'";
            $connection = ConnectionManager::get('default');
            $queryy = $connection->execute("SELECT reglements.id FROM factures,reglements,lignereglements WHERE lignereglements.reglement_id = reglements.id and lignereglements.facture_id = factures.id and $condd ")->fetchAll('assoc');
            // debug($queryy);
            $ids =[];
            foreach ($queryy as $qr) {
                $ids[] = $qr['id'];
            }
            if(!empty($ids)) $idss=$ids; else $idss=0;
            $conddd = ['Reglements.id IN' => $idss];
        }
        $query = $this->Reglements->find('all', [
            'conditions' => $conddd,
            $cond, // Ajouter les conditions supplémentaires ici
            'contain' => ['Fournisseurs'],
            'order' => ['Reglements.id' => 'DESC'],
        ]);
        $cmd = $this->paginate($query);
    ...
    public function add($four = null, $fac = null)
    {
        ...
        if ($fac != 0) {
            ...
            $paiementfacturesIds = array_merge($paiementfacturesIds, [5, 7, 8]);
    ...
    public function edit($id = null)
    {
        ...
        if ($lignesreg->facture_id!=0){
        $paiementfactures = $this->fetchTable('Paiementfactures')->find()->where('Paiementfactures.facture_id=' . $lignesreg->facture_id);
        $paiementfacturesIds = [];
        foreach ($paiementfactures as $row) {
            $paiementfacturesIds[] = $row->paiement_id;
        }
    ...
    public function addlibre($fournisseur_id = null, $piece)
    {
        error_reporting(E_ERROR | E_PARSE);
        //debug($fournisseur_id);
        $this->loadModel('Factures');
        ...
        if ($this->request->is('post')) {
            ...
            $data['fournisseur_id'] = $this->request->getData('fournisseur_id');
            $data['pointdevente_id'] = $this->request->getData('pointdevente_id');
            $data['Montant'] = $this->request->getData('Montant');
            $data['libre'] = 1;
        ...
        $this->loadModel('Paiements');
        $paiements = $this->Paiements->find('list');
        $this->loadModel('Carnetcheques');
        ...
        //debug($piece->toArray());
        $cha = "TRUE";
        $fournisseurs = $this->Reglements->Fournisseurs->find('all');
        ...
        $this->set(compact('comptes','piece', 'commandes', 'pointdeventes', 'caisses', 'banques', 'timbre', 'type', 'valeurs', 'carnetcheques', 'paiements', 'livraisons', 'factures', 'fournisseur_id', 'code', 'reglement', 'fournisseurs'));
    }
}
