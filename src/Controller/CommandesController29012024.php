 ...
 */
    ...
    public function index()
    {
        ...
        // if (!$etat) {
        $query = $this->Commandes->find('all')->where([$cond2, $cond3, $cond4, $cond5, $cond6, $cond7, $cond8, $condtype, $cond9999, $condcommercial, $condetatm])->order(['Commandes.id' => 'DESC']);
        // } else {
        ...
        ///debug($query);die;
        $this->paginate = [
            'contain' => ['Clients', 'Commercials', 'Etattransports', 'Depots'],
        ];
        ...
        $this->set(compact('depots', 'suivi', 'datefin', 'datedebut', 'etat', 'article', 'articles', 'zones', 'clients', 'commandes', 'commercials', 'clientsoptions', 'commercialsoptions', 'numero', 'client_id', 'commercial_id'));
    }
    ...
    public function view($id = null)
    {
        ...
        $lignereglementclientoffre = $this->fetchTable('Lignereglementclients')->find('all')->where(['Lignereglementclients.bonlivraison_id =' . $bl_id])->first();
        $piecereglementclientoffres = [];
        if ($lignereglementclientoffre->reglementclient_id) {
            $piecereglementclientoffres = $this->fetchTable('Piecereglementclients')
                ->find('all')
                ->where(['Piecereglementclients.reglementclient_id =' . $lignereglementclientoffre->reglementclient_id])
                ->contain(['Caisses', 'Banques', 'Paiements', 'Tos']);
        }


        $lignereglementclient = $this->fetchTable('Lignereglementclients')->find('all')->where(['Lignereglementclients.commande_id =' . $id])->first();
        $piecereglementclients = [];
        if ($lignereglementclient->reglementclient_id != null) {
            $piecereglementclients = $this->fetchTable('Piecereglementclients')
                ->find('all')
                ->where(['Piecereglementclients.reglementclient_id =' . $lignereglementclient->reglementclient_id])
                ->contain(['Caisses', 'Banques', 'Paiements', 'Tos']);
        }
        ...
        $this->set(compact('etattransports', 'piecereglementclientoffres', 'banques', 'caisses', 'valeurs', 'paiements', 'piecereglementclients', 'champ', 'BL', 'gs', 'not', 'remes', 'remcli', 'es', 'rz', 'cl', 'typecl', 'exotva', 'exofodec', 'exotimbre', 'exotpe', 'clientc', 'lignecommandes', 'commande', 'depots', 'clients', 'commercials', 'articles', 'timbre', 'pointdeventes', 'escompte', 'type'));
     }
    ...
    public function addcommande($tab = null)
    {
        ...
        $currentYear = date('y');
        $num = $this->Commandes->find()->select(["num" =>
        'MAX(Commandes.numero)'])->where('YEAR(Commandes.date)=' . $yearf)->first();
        $n = $num->num;
        ...
        $lignereglementclient = $this->fetchTable('Lignereglementclients')->find('all')->where(['Lignereglementclients.bonlivraison_id =' . $tab])->first();
        ...
        $this->set(compact('etattransports', 'piecereglementclientoffres', 'banques', 'piecereglementclients', 'caisses', 'valeurs', 'paiements', 'cl', 'escompte', 'exotva', 'exofodec', 'exotpe', 'bonus', 'clientc', 'dates', 'commande', 'bonlivraison', 'lignebonlivraisons', 'mm', 'articles', 'commercials', 'clients', 'depots', 'es', 'rz', 'typecl', 'remcli', 'not'));
    }
