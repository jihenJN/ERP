...
class ClientsController extends AppController {
    ...
    public function index() {
        ...
        $this->paginate = [
           /* 'contain' => ['Gouvernorat'],*/ 'order' => ['left(Clients.Code,1),cast(right(Clients.Code,length(Clients.Code)-1) as Unsigned)', 'id' => 'ASC']
        ];
          ...
          //  debug($this->request->getData());
            if ($this->Clients->save($client)) {
               // debug($client);
                ...
                $prixcli = $this->Clientarticles->find('all', [])
                        ->where(["Clientarticles.client_id  ='" . $id . "'"])
                        ;
                foreach ($prixcli as $c) {
                    $this->Clientarticles->delete($c);
                }
                ...
                if (isset($this->request->getData('data')['responsable']) && (!empty($this->request->getData('data')['responsable']))) {
                    ...
                    foreach ($this->request->getData('data')['lignes'] as $i => $exon) {
                        ...
                        if ($exon['sup2'] != 1) {
                            ...
                            if ($this->fetchTable('Clientexonerations')->save($exonerations)) {
                            } else {
...
//                ->where(['Delegations.id  in (' . $tab . ')']);
       ...
       //debug($clientarticles);
        ...
        $this->set(compact('bureaupostes','localite','deleg','type','tab','articles','clientarticles','exoner','pointdeventes', 'typeclients', 'client', 'commercials', 'gouvernorats', 'typeexonerations', 'paiements', 'adressees', 'responsable', 'banquess', 'banques'));
...
//                            $targetPath = WWW_ROOT . 'img' . DS . 'imgart' . DS . $name;
                      ...
                      //debug($exonerations);
                            ...
                            if ($this->Clientexonerations->save($exonerations)) {
                              //debug($exonerations);
                            }
                        } else {
                        }
...
//debug($tab);
        ...
        $this->set(compact('client','tab','articles','type','delegations','localites','commercials', 'gouvernorats', 'typeexonerations', 'banques', 'paiements', 'typeclients', 'pointdeventes', 'bureaupostes'));
    }
...
//                ->where(['Delegations.id  in (' . $tab . ')']);
       ...
       //debug($clientarticles);
        ...
        // debug($bureaupostes);
        $commercials = $this->Clients->Commercials->find('list', ['limit' => 200])->all();
        $this->set(compact('bureaupostes','localite','deleg','gouvernorats','type','tab','articles','clientarticles','exoner','gouvernorats','pointdeventes', 'typeclients', 'client', 'commercials', 'gouvernorats', 'typeexonerations', 'paiements', 'adressees', 'responsable', 'banquess', 'banques'));
    }
    ...
    public function getbureaupostegouvs($id = null) {
      ...
      $select = "
       ...
       echo json_encode(array("query" =>$code,"queryy" =>$c,"select" =>$select, "success" => true));
     ...
     $select = "
       ...
       echo json_encode(array("query" =>$code,"select" =>$select, "success" => true));
...
//        echo json_encode(array('select' => $select));
  ...
  //  debug($code);die;
echo json_encode(array("query" =>$code, "success" => true));
