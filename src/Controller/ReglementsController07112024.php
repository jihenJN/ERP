...
class ReglementsController extends AppController
{
    ...
    public function view($id = null)
    {
        ...
        $four = $reglement->fournisseur_id;
        $p = $reglement->pointdevente_id;
        $difference =  $reglement->differance;
        $idd = $id;
        // debug( $difference);
        $lignesreg = $this->Lignereglements->find('all')->where(['Lignereglements.reglement_id =' . $id]);

        $piecereglements = $this->Piecereglements->find('all')->where(['Piecereglements.reglement_id =' . $id,'Piecereglements.paiement_id not in(6,7,9)']);
        $mtbon = 0.000;
        $mtfact = 0.000;
        foreach ($lignesreg as $k => $ligne) {
            if ($ligne['facture_id'] != null) {
        ...
        $cheques = $this->fetchTable('Cheques')->find('list', ['keyfield' => 'id', 'valueField' => 'numero']);
        $this->set(compact('mtfact', 'carnetcheques', 'banques', 'comptes', 'cheques', 'mtbon', 'idd', 'facreg', 'difference', 'piecereglements', 'lignesreg', 'pointdeventes', 'valeurs', 'carnetcheques', 'paiements', 'p', 'four', 'livraisons', 'factures', 'reglement', 'fournisseurs', 'importations', 'utilisateurs', 'exercices', 'devises'));
    }
    /**
     * Add method
     *
    ...
    public function add($four = null, $p = null)
    {
        ...
        $exercices = $this->Reglements->Exercices->find('list', ['limit' => 200])->all();
        $devises = $this->Reglements->Devises->find('list', ['limit' => 200])->all();
        $this->set(compact('valeurs', 'paiements', 'livraisons', 'banques', 'factures', 'four', 'p', 'code', 'reglement', 'carnetcheques', 'pointdeventes', 'fournisseurs', 'utilisateurs', 'exercices', 'devises'));
    }
    ...
    public function edit($id = null)
    {
        ...
        $cheques = $this->fetchTable('Cheques')->find('list', ['keyfield' => 'id', 'valueField' => 'numero']);
        $this->set(compact('mtfact', 'mtbon', 'idd', 'banques', 'comptes', 'cheques', 'facreg', 'piecereglements', 'lignesreg', 'pointdeventes', 'carnetcheques', 'valeurs', 'paiements', 'p', 'four', 'livraisons', 'factures', 'reglement', 'fournisseurs', 'importations', 'utilisateurs', 'exercices', 'devises'));
    }
