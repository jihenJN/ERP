...
class CommandefournisseursController extends AppController
{
    ...
    public function index($type = null)
    {
        ...
        $query = $this->Commandefournisseurs->find('all')->where([$condtype, $cond1, $cond1, $cond2, $cond3, $cond4, $cond5, $cond6, $cond7, $cond8, $cond9, $cond10])
            ->order(['Commandefournisseurs.id' => 'DESC']);
        //  debug($query);
    ...
    public function add($type = null)
    {
        ...
        $articles = $this->Commandefournisseurs->Articles->find('all')
            ->where([$cond]);
        $services = $this->fetchTable('Services')->find('list', ['keyfield' => 'id', 'valueField' => 'name']);
        $machines = $this->fetchTable('Machines')->find('list', ['keyfield' => 'id', 'valueField' => 'name']);
        // debug($articles);
        //debug($articles);die();
        $this->set(compact('machines','services','commande', 'articles', 'demandeoffredeprixes', 'fournisseurs', 'pointdeventes', 'depots', 'cartecarburants', 'materieltransports', 'type'));
    }
    ...
    public function addbonliv($id = null)
    {
        ...
        $lignes = $this->Commandefournisseurs->Lignecommandefournisseurs->find('all', [
            'contain' => ['Articles']
        ])
            ->where(['commandefournisseur_id' => $id]);
        // $lignes = $this->Lignecommandefournisseurs->find('all', ['contain' => ['Articles']])
