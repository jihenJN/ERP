...
class TracemisejoursController extends AppController
{
    ...
    public function index()
    {
        ...
        if ($utilisateur_id) {
            $cond3 = "Tracemisejours.utilisateur_id   like  '%" . $utilisateur_id . "%' ";
        } 
        if ($historiquede) {
            $cond2="Tracemisejours.date like '%".$historiquede."%'";
        }
       if ($au) {
           $cond3= "Tracemisejours.date like'%".$au."%'";
       }

        $query = $this->Tracemisejours->find('all')->where([$cond1, $cond2,$cond3,$cond4,$cond5]);
        ...
        $this->set(compact('tracemisejours','utilisateurs','recherches')); 
    }
