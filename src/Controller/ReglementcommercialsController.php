...
//        $cond2 = '';
        ...
        $cond100 = 'Lignebonlivraisons.montantcommission  >0';
        $lignelivraison = $this->fetchTable('Lignebonlivraisons')->find('all')
                ->where([$cond1, $cond2,$cond100]);
        $s = 0;
        $restbonus = 0;
        ...
        if ($this->request->is('post')) {
           // debug($this->request->getData());//die;
            ...
            if ($this->Reglementcommercials->save($reglementcommercial)) {
                ...
                $rg_id = ($this->Reglementcommercials->save($reglementcommercial)->id);
                $this->misejour("Reglementcommercials", "add", $rg_id);
...
//        $cond2 = '';
        $cond1 = 'Lignebonlivraisons.bonlivraison_id   in (' . $list . ')';
         $cond50 = 'Lignebonlivraisons.montantcommission  >0';
        $cond2 = 'Lignebonlivraisons.commission  ="FALSE" or '.$condliv;
        $lignelivraison = $this->fetchTable('Lignebonlivraisons')->find('all')
                ->where([$cond1, $cond2,$cond50]);
        $s = 0;
        $restbonus = 0;
...
//                                    debug($dat['montantentre']);
        ...
        if ($this->request->is(['patch', 'post', 'put'])) {
             ...
             foreach ($ligne as  $t)
            {
                ...
                if ($t['lignebonlivraison_id'] !=0)
                { 
                    $li=$this->fetchTable('Lignebonlivraisons')->find()->where(['Lignebonlivraisons.id ='.$t['lignebonlivraison_id']])->first();
                    if ($li['montantcommission'] == $t['montant']) {
                                $li->commission = 'FALSE';
                                $this->fetchTable('Lignebonlivraisons')->save($li);
                            }
               ...
               if ($t['lignebonusmalu_id'] !=0)
                {
                   $lig=$this->fetchTable('Lignebonusmalus')->find('list')->where(['Lignebonusmalus.id ='.$t['lignebonusmalu_id']])->first();
                    if ($lig['montant'] == $t['montant']) {
                                $lig->paye = 0;
                                $this->Lignebonusmalus->save($lig);
                            }
     ...
     // die;
       ...
       //   debug($reglementcommercial);//die;
           if ($this->Reglementcommercials->save($reglementcommercial)) {
            ...
            $rg_id = ($this->Reglementcommercials->save($reglementcommercial)->id);
                $this->misejour("Reglementcommercials", "edit", $rg_id);
               // debug($reglementcommercial);die;
                $id_reg = $reglementcommercial->id;
           $li = array();
                if (!empty($this->request->getData('data')['reglement'])) {
        ...
        $lignereglement = $this->fetchTable('Lignereglementcommercials')->find('all')
             ...
             foreach ($ligne as  $t)
            {
                ...
                { 
                     $li=$this->fetchTable('Lignebonlivraisons')->find()->where(['Lignebonlivraisons.id ='.$t['lignebonlivraison_id']])->first();
                    if (
                            $li['montantcommission'] == $t['montant']) {
                                $li->commission = 'FALSE';
                                $this->fetchTable('Lignebonlivraisons')->save($li);
                            }
                     $lignebonlivraison = $this->fetchTable('Lignebonlivraisons')->get($t['lignebonlivraison_id']);
               ...
               if ($t['lignebonusmalu_id'] !=0)
                {
                   ...
                   $lig=$this->fetchTable('Lignebonusmalus')->find('list')->where(['Lignebonusmalus.id ='.$t['lignebonusmalu_id']])->first();
                    if ($lig['montant'] == $t['montant']) {
                                $lig->paye = 0;
                                $this->fetchTable('Lignebonusmalus')->save($lig);
                            }
...
//            $this->Reglementcommercials->save($reglementcommercial);
        ...
        if ($this->Reglementcommercials->delete($reglementcommercial)) {
            ...
            $rg_id = ($this->Reglementcommercials->save($reglementcommercial)->id);
                $this->misejour("Reglementcommercials", "delete", $rg_id);
    ...
    public function contenureglement() {
        ...
        $cond20 = 'Lignebonlivraisons.montantcommission  >0';
        $lignelivraison = $this->Lignebonlivraisons->find('all')
                ->where([$cond1, $cond2,$cond20]);
