...
//    }
 public function addindirect($tab = null)
   ...
   $chauffeurs = $this->Personnels->find('all')->where(["Personnels.fonction_id  ='" . 5 . "%' "]);
      ...
      $this->set(compact('lignelivraisons','tab1','timbr','livraison', 'numero', 'articles', 'facture', 'fournisseurs', 'pointdeventes', 'depots', 'materieltransports', 'cartecarburants', 'chauffeurs', 'conffaieurs', 'adresselivraisonfournisseurs'));
    }
     ...
     */
    public function index( $typef = null)
    {
        ...
        $condtype = "Factures.typefacture=" . $typef;


        $this->paginate = [
            'contain' => ['Fournisseurs', 'Adresselivraisonfournisseurs', 'Pointdeventes', 'Depots', 'Cartecarburants', 'Materieltransports'],
        ];
        $query = $this->Factures->find('all')->where([$condtype,$cond1, $cond2, $cond3, $cond4, $cond5, $cond6])
     ->order(["Factures.id"=>'DESC']);
        ...
        $this->set(compact('typef','confaieurs', 'factures', 'fournisseurs', 'chauffeurs', 'materieltransports', 'pointdeventes', 'depots', 'cartecarburants'));
    }
     ...
     */
    public function add($typef = null)
    {
    ...
    {$session = $this->request->getSession();
        ...
        if ($this->request->is(['patch', 'post', 'put'])) {
            ...
            ->where(['facture_id' => $id]);
              ...
              return $this->redirect(['action' => 'index/' . $facture['typefacture']]);
            }
          //  $this->Flash->error(__('The {0} could not be saved. Please, try again.', 'Facture'));
        }
        //$commandes = $this->Factures->Commandes->find('list', ['limit' => 200]);
