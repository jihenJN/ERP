...

{
    public function getcode()
    {
        ...
        $code = $this->request->getQuery('client');
        $index = $this->request->getQuery('index');

        // Initialisation de la variable $select
        $select = '';

        // Vérification si le code est fourni
        if ($code) {
            // Récupération de l'article correspondant au code
            $articleTable = $this->fetchTable('Articles');
            $articlee = $articleTable->find()
                ->where(['Articles.Dsignation' => $code])
                ->first();
            ...
            if ($articlee) {
                ...
                $query = $articleTable->find('all', [
                    'fields' => ['Code'] // Spécifiez les champs nécessaires
                ]);

                // Construction du HTML pour l'élément <input> et <datalist>
                //  $select = "<input class='form-control' value=\"" . htmlspecialchars($des, ENT_QUOTES, 'UTF-8') . "\" list=\"desarticle_id" . htmlspecialchars($index, ENT_QUOTES, 'UTF-8') . "\" champ='article_idcode' index=\"" . htmlspecialchars($index, ENT_QUOTES, 'UTF-8') . "\" id=\"article_idcode" . htmlspecialchars($index, ENT_QUOTES, 'UTF-8') . "\" onchange=\"getdesignation(this.value,$index)\" type=\"text\" />";
                //  $select .= "<datalist champ='codearticle_id' id=\"codearticle_id" . htmlspecialchars($index, ENT_QUOTES, 'UTF-8') . "\">";
                $select = "";
                // Ajout des options au <datalist>
                foreach ($query as $article) {
                    $isSelected = ($article->Code === $des) ? ' selected' : '';
                    $select .= "<option" . $isSelected . " value=\"" . htmlspecialchars($article->Code, ENT_QUOTES, 'UTF-8') . "\"></option>";
                }
        ...
        echo json_encode(['select' => $select, 'value' => htmlspecialchars($des, ENT_QUOTES, 'UTF-8')]);
        exit;
    }
    ...
    public function getdesignation()
    {
        ...
        if ($code) {
            ...
            // Vérifiez si l'article existe
            if ($articlee) {
                $des = $articlee->Dsignation;
                // Génération de la liste déroulante
                $query = $articleTable->find('all', [
                    'fields' => ['Dsignation'] // Spécifiez les champs nécessaires
                ]);

                // Construction du HTML pour l'élément <input> et <datalist>
                //  $select = "<input class='form-control' name='data[ligner][$index][desarticle_id]' value=\"" . htmlspecialchars($des, ENT_QUOTES, 'UTF-8') . "\" list=\"desarticle_id" . htmlspecialchars($index, ENT_QUOTES, 'UTF-8') . "\" champ='article_iddes' index=\"" . htmlspecialchars($index, ENT_QUOTES, 'UTF-8') . "\" id=\"article_iddes" . htmlspecialchars($index, ENT_QUOTES, 'UTF-8') . "\" onchange=\"getcode(this.value,$index)\" type=\"text\" />";
                // $select .= "<datalist  name='data[ligner][$index][desarticle_id]' champ='desarticle_id' id=\"desarticle_id" . htmlspecialchars($index, ENT_QUOTES, 'UTF-8') . "\">";
                $select = "";
                // Ajout des options au <datalist>
                foreach ($query as $article) {
                    $isSelected = ($article->Dsignation === $des) ? ' selected' : '';
                    $select .= "<option" . $isSelected . " value=\"" . htmlspecialchars($article->Dsignation, ENT_QUOTES, 'UTF-8') . "\"></option>";
                }
    ...
    public function adddalandat($type = null)
    {
        ...
        $timbre = $tim->timbre;
        $this->set(compact('escompte', 'timbre', 'paiements', 'mm', 'transporteurs', 'type', 'valeurs', 'caisses', 'banques', 'commercials', 'type', 'dates', 'mm', 'articles', 'bonlivraison', 'clients', 'depots', 'materieltransports', 'cartecarburants', 'chauffeurs', 'conffaieurs', 'factureclients', 'adresselivraisonclients'));
    }
    public function add($type = null)
    {
        ...
        $this->set(compact('escompte', 'timbre_id', 'timbre_max', 'timbre', 'paiements', 'mm', 'transporteurs', 'type', 'valeurs', 'caisses', 'banques', 'commercials', 'type', 'dates', 'mm', 'articles', 'bonlivraison', 'clients', 'depots', 'materieltransports', 'cartecarburants', 'chauffeurs', 'conffaieurs', 'factureclients', 'adresselivraisonclients'));
    }
    ...
    ///////////////
    public function haithamselectt()
    {

        //print_r($this->request->getQuery('val'));


        $data = $this->request->getQuery('val');

        $this->loadModel('Articles');
        $connection = ConnectionManager::get('default');
        //$pp = $this->Articles->find('all')->where(['Articles.Dsignation LIKE %'.$data.'%']);
        $prix = $connection->execute("SELECT * FROM articles where articles.Code LIKE '%$data%'; ")->fetchAll('assoc');
    ...
    public function haithamselect()
    {
        ...
        $connection = ConnectionManager::get('default');
        //$pp = $this->Articles->find('all')->where(['Articles.Dsignation LIKE %'.$data.'%']);
        $prix = $connection->execute("SELECT * FROM articles where articles.Dsignation LIKE '%$data%'; ")->fetchAll('assoc');
    ...
    public function add09092024($type = null)
    {
    ...
    public function imprimeproforma($id = null)
    {
        ...
        $bonlivraison = $this->Bonlivraisons->get($id, [
            'contain' => ['Clients', 'Depots', 'Timbres'],
        ]);
        ...
        $this->set(compact('commercials', 'timbre', 'tab', 'commercial', 'exotpe', 'exotva', 'exofodec', 'exotimbre', 'bonus', 'type', 'client', 'lignebonlivraisons', 'articles', 'bonlivraison', 'clients', 'depots', 'materieltransports', 'cartecarburants', 'chauffeurs', 'conffaieurs', 'adresselivraisonclients'));
    }
    public function imprimebacg($id = null)
    {
    ...
    public function imprimeviewsmbm($id = null)
    {
        ...
        // var_dump($lignebonlivraisons->toArray());
        //debug($bonlivraison);die;
    ...
    public function getids()
    {
        ...
        $bonlivraison = $this->fetchTable('Bonlivraisons')->find('all', [
            ...
            ->select([
                ...
                'numero' =>  'Bonlivraisons.numero',
                'observation' => 'Bonlivraisons.observation',
        ...
        foreach ($bonlivraison as $bb) {
            ...
            $totalttc = $bb->totalttc + $timbre_max;
            $observation = $bb->observation;
            $totalputtc = $bb->totalputtc;
            $bonlivraison_id = $bb->id;
            $type = 1;
            $numeroidentite = $bb->numeroidentite;
            $adressediv = $bb->adressediv;
            $nomprenom = $bb->nomprenom;

            ////////////////////reglement 1 dinar timbre //////////////////////////////////


            /* $lignereg = $this->fetchTable('Lignereglementclients')
                ->find()
                ->select(['total' => $this->fetchTable('Lignereglementclients')->find()->func()->sum('Montant')])
                ->where(['bonlivraison_id' => $bb->id])
                ->first();
            // debug($lignereg->total);die;
            if ($lignereg && $lignereg->total == $bb->totalttc) {
                ...
                // $connection->execute("INSERT INTO reglementclients (INSERT INTO `reglementclients` (`client_id`, `date`, `Montant`, `montantaffecte`, `pointdevente_id`, `numero`, `numeroconca`, `SR`, `NB`, `NumNB`, `client`, `mg`, `exercice_id`, `utilisateur_id`, `type`, `affectation_id`, `emi`, `numeropieceintegre`, `group_id`, `differance`, `observation`, `libre`, `user_id`, `dif`) 
                // VALUES ($client_id, '$datereg',$timbre , 0.000, 0, '$nume', '$code', NULL, NULL, NULL, NULL, NULL, 0, 0, 1, 0, '0', '0', 0, 0, NULL, 0, $user_id, 1)");

                $connection->execute("INSERT INTO `reglementclients` (`client_id`, `date`, `Montant`, `montantaffecte`, `pointdevente_id`, `numero`, `numeroconca`, `SR`, `NB`, `NumNB`, `client`, `mg`, `exercice_id`, `utilisateur_id`, `type`, `affectation_id`, `emi`, `numeropieceintegre`, `group_id`, `differance`, `observation`, `libre`, `user_id`, `dif`) 
             VALUES ($client_id, '$datereg', $timbre, 0.000, 0, '$nume', '$code', NULL, NULL, NULL, NULL, NULL, 0, 0, 1, 0, '0', '0', 0, 0, NULL, 0, $user_id, 1)");

                $maxIdReg = $connection->execute("SELECT MAX(id) AS max_idreg FROM reglementclients")->fetch('assoc');
                $maxIdr = $maxIdReg['max_idreg'];
                // $Montantreg = $maxIdReg['Montant'];


                $connection->execute("INSERT INTO lignereglementclients (`reglementclient_id`, `Montant`, `factureclient_id`, `piecereglementclient_id`, `remise`, `SR_BL`, `NB_BL`, `affectation_id`, `bonlivraison_id`, `commande_id`, `client_id`) 
        
                VALUES ($maxIdr, '$timbre', 0, 0, NULL, NULL, NULL, NULL, $bonlivraison_id, NULL, NULL)");
                $maxIdligne = $connection->execute("SELECT MAX(id) AS max_idligne FROM lignereglementclients")->fetch('assoc');
                // $maxIdlig = $maxIdligne['max_idligne'];




                $connection->execute("INSERT INTO piecereglementclients (`paiement_id`, `reglementclient_id`, `montant`, `num`, `echance`, `banque`, `montant_brut`, `montant_net`, `to_id`, `compte`, `situation`, `datesituation`, `reglement`, `mantantregler`, `emi`, `envoye`, `valide`, `valeur`, `etat_id`, `numeropieceintegre`, `prop`, `commission`, `tvacommission`, `caisse_id`, `acomptetype`, `piecejointe`, `rib`, `porteurcheque`, `endosse`, `echance2`, `compte_id`, `banque_id`) 
                 VALUES (9, $maxIdr, $timbre,NULL, NULL, NULL, NULL, NULL, NULL, NULL, 'En attente', NULL, 0, 0.000, 0, 0, 0, NULL, 1, '0', NULL, 0.000, 0.000, NULL, 1, NULL, NULL, NULL, NULL, NULL, NULL, NULL)");
            } else {
            }*/
            ...
            $connection->execute("INSERT INTO factureclients (numero, date, client_id, depot_id, totalht, totaltva, totalfodec, totalremise, totalttc, observation, user_id, totalputtc ,timbre_id,bonlivraison_id,type,numeroidentite,adressediv,nomprenom) 
                VALUES ($numero, '$date', $client_id, $depot_id, $totalht, $totaltva, $totalfodec, $totalremise, $totalttc, '$observation', $user_id, $totalputtc ,$timbre_id,$bonlivraison_id,$type,'$numeroidentite','$adressediv','$nomprenom')");
            ...
            foreach ($lignebonlivraisons as $l) {
                ...
                $puttc = $l->puttc;
                $puttcapr = $l->puttcapr;
                $ttchidden = $l->ttchidden;

                //  $totalttc = $l->totalttc;

                $connection->execute("
                INSERT INTO lignefactureclients 
                (factureclient_id, article_id, qte, punht, prixht, ml, remise, fodec, tva, ttc, puttc, lignebonlivraison_id, puttcapr, ttchidden)
                VALUES 
                ($factureclient_id, $article_id, $qte, $punht, $prixht, $ml, '$remise', '$fodec', $tva, '$ttc', $puttc, $lignebonlivraison_id, '$puttcapr', '$ttchidden')
            ")->fetchAll('assoc');


                // $connection->execute("INSERT INTO lignefactureclients (factureclient_id,article_id, qte, punht, prixht, ml, remise, fodec, tva,ttc,puttc ,lignebonlivraison_id,'puttcapr','ttchidden') 
                // VALUES ($factureclient_id,$article_id,$qte,$punht,$prixht,$ml,'$remise','$fodec',$tva,'$ttc',$puttc,$lignebonlivraison_id,'$puttcapr','$ttchidden')
                // ;")->fetchAll('assoc');
            }
        }
        // die;
        echo json_encode(array(
            'jbn' => 0,
            //'factureIds' => $factureIds,  // Send back the IDs of the created factures
    ...
    public function getchequeclient()
    {
        $clientId = $this->request->getQuery('client_id');

        $this->loadModel('Piecereglementclients');
        $this->loadModel('Etatreglementclients');
        ...
        if ($clientId) {
            ...
            $subquery = $this->Piecereglementclients->Etatreglementclients->find()
                ...
                ->andWhere(['Reglementclients.client_id' => $clientId])
                //    ->andWhere(['Reglementclients.date <=' => $Date])
            ...
            $piecereglements = $this->Piecereglementclients->find('all')
                ...
                ->where(['Piecereglementclients.id IN' => $subquery])
        ...
        $res = "<br><table style='border: 1px solid black; width: 100%; border-collapse: collapse;'>
        <thead>
            <tr style='background-color: #C98181;'>
                <td align='center' style='width: 25%; border: 1px solid black;'><strong>Numéro Chèque</strong></td>
                <td align='center' style='width: 8%; border: 1px solid black;'><strong>Échéance</strong></td>
                <td align='center' style='width: 12%; border: 1px solid black;'><strong>Montant</strong></td>
            </tr>
        </thead>";

        foreach ($piecereglements as $q) {
            $res .= "<tbody><tr style='background-color: #C98181;'>";
            $res .= "<td style='border: 1px solid black;'><input value='" . $q->num . "' type='number' class='form-control' readonly></td>";
            $res .= "<td style='border: 1px solid black;'><input value='" . $q->echance . "' type='text' class='form-control' readonly></td>";
            $res .= "<td style='border: 1px solid black;'><input value='" . $q->montant . "' type='number' class='form-control' readonly></td>";
            $res .= "</tr></tbody>";
        }
        ...
        echo json_encode([
            'res' => $res,
            'longueur' => count($piecereglements) // Assuming 'longueur' means the count of cheques
        ]);
        exit;
    }
    public function getchequeclient31102024()
    {
        $clientId = $this->request->getQuery('client_id');
        ...
        if ($clientId) {
            ...
            if (!empty($piecereglements)) { // Only generate the table if there are cheques
                $res = "<br><table style='border: 1px solid black; width: 100%; border-collapse: collapse;'>
                <thead>
                    <tr style='background-color: #C98181;'>
                        <td align='center' style='width: 25%; border: 1px solid black;'><strong>Numéro Chèque</strong></td>
        ...
        echo json_encode([
            'res' => $res,
            'longueur' => count($piecereglements) // Count of cheques
        ]);
        exit;
    }


    public function getchequeclient1410()
    {
        $clientId = $this->request->getQuery('client_id');
        $response = ['piecereglements' => []]; // Initialize the response
        ...
        if ($clientId) {
            ...
            $subquery = $this->Piecereglementclients->Etatreglementclients->find()
                ...
                ->where(['Etatreglementclients.etat_id' => 3])
                ->andWhere(['Reglementclients.client_id' => $clientId])
                ->andWhere(['Reglementclients.date <=' => $currentDate])
                ->distinct(['piecereglementclient_id']);
            ...
            $response['piecereglements'] = $piecereglements;
        }
            ...
            if ($this->Bonlivraisons->save($bonlivraison)) {
                ...
                if ($this->request->getData('Montant_Regler') != '0' || $this->request->getData('Montant_Regler') != 0) {
                    ...
                    $numeroobj = $this->fetchTable('Reglementclients')->find()->select([
                        "numero" =>
                        'MAX(Reglementclients.numeroconca)'
                    ])->where(['Reglementclients.type=1'])->first();
                    $numero = $numeroobj->numero;
                    if ($numero != null) {
                        $n = $numero;
                        $lastnum = $n;
...
<?php
    ...
    public function getclientdata()
    {
        ...
        if ($client) {
            ...
            $clientfirsts = $this->fetchTable('Clients')->find('all')->where(['Clients.id' => $client])->first();

            $ss = $scl[0]['s'] + $clientfirsts->soldedebut;
    ...
    public function etatjournalier()
    {
        ...
        $reglementstot = $this->Piecereglementclients->find()->select(['montant', 'reglementclient_id', 'paiement_id', 'num'])->all(); //->where(['caisse_id' => 5])->all();
        ...
        $reglementsini = $this->Piecereglementclients->find()->select(['montant', 'reglementclient_id', 'paiement_id', 'num'])->where([$condp1])->all();
        // debug($reglementsini);
        ...
        $pieceregini = $connection->execute("
                    ...
                    JOIN reglementclients ON piecereglementclients.reglementclient_id = reglementclients.id
                WHERE
                    date(reglementclients.date) < '" . $historiquede . "' AND " . $condppiece . "")->fetchAll('assoc');
        ...
        if ($historiquede != null) {
            ...
            $clientData[] = [
                ...
                'index' => '1',
                'date' =>  date('Y-m-d', strtotime($historiquede . ' -1 day')),
                //'date' => $thiscaisse->date,
            ];
        }
        ...
        foreach ($reglements as $reglement) {
            ...
            if ($reglement_id != null) {
                $reglementclient = $this->Reglementclients->find()
                    ->select(['client_id', 'date', 'numeroconca', 'montant'])
                    ->where(['id' => $reglement_id, $conditions])
                    ->first();

                $ligneregs = $this->Lignereglementclients->find()
                    ->select(['Montant', 'bonlivraison_id', 'commande_id', 'factureclient_id']) //->contain(['Bonlivraisons','Commandes','Factureclients'])
                    ->where(['Lignereglementclients.reglementclient_id' => $reglement_id]);

                foreach ($ligneregs as $key => $lignereg) {
                    if ($lignereg['bonlivraison_id']) {
                        $numeroBon = $this->Bonlivraisons->find()
                            ->select(['numero'])
                            ->where(['id' => $lignereg->bonlivraison_id])
                            ->first();
                        $numfact[$key] = ' BL N° ' . $numeroBon['numero'];
                    }
                    if ($lignereg['factureclient_id']) {
                        $numeroFAC = $this->Factureclients->find()
                            ->select(['numero'])
                            ->where(['id' => $lignereg->factureclient_id])
                            ->first();
                        $numfact[$key] = ' FAC N° ' . $numeroFAC['numero'];
                    }
                    if ($lignereg && $lignereg->commande_id != null) {
                        $cmd = $this->fetchTable('Commandes')->find()
                            ->where(['id' => $lignereg->commande_id])
                            ->first();
                        $numfact[$key] = ' CMD N° ' . $cmd['numero'];
                    }
                }

                // if ($lignereg && $lignereg->bonlivraison_id != null) {
                //     $numeroBon = $this->Bonlivraisons->find()
                //         ->select(['numero'])
                //         ->where(['id' => $lignereg->bonlivraison_id])
                //         ->first();
            ...
            if ($reglementclient) {
                $client = $this->Clients->find()
                    ...
                    ->where(['id' => $reglementclient->client_id])
                    ->first();
            }

            $paiement = [];
            if ($paiement_id) {
                $paiement = $this->Paiements->find()
                    ->select(['name'])
            ...
            if (isset($client->Raison_Sociale) && $reglementclient && $reglementclient->date) {
                $numbl = '';
                if ($lignereg && $lignereg->bonlivraison_id != null) {
                    $bl = $this->fetchTable('Bonlivraisons')->find()
                        ->where(['id' => $lignereg->bonlivraison_id])
                        ->first();
                    $numbl = $bl->numero;
                    $namediver = $bl->nomprenom;
                    // debug($namediver);
                }
                ...
                $numfac = '';
                if ($lignereg && $lignereg->factureclient_id != null) {
                    $facc = $this->fetchTable('Factureclients')->find()
                        ->where(['id' => $lignereg->factureclient_id])
                        ->first();
                    $numfac = $facc->numero;
                }
                ...
                $clientData[] = [
                    ...
                    'montant' => $reglementclient->montant,
                    'montantpiece' => $reglementclient->montant,
        ...
        foreach ($transfertdeparts as $transfertdepart) {
            ...
            $clientData[] = [
                'caissedep' => $caissedepart->name,
                'caissearr' => $compte->numero,
                'name' => '',
                'date' => $transfertdepart->date,
                'paiement_name' => $paiement->name,
                'credit' => $transfertdepart->montant,
                'debit' => '',
                'code' => '',
                'num' =>  $transfertdepart->numero,
                'observation' => $transfertdepart->observation,
                'type' => 'Transfert caisse/compte',
                'index' => '4',
        ...
        $this->set(compact('clientData', 'credit_all', 'debit_all', 'paiements'));
    }
    public function etatcaisse()
    {
        ...
        // if ($paiement_id) {
        // $condp1[] = ["Piecereglementclients.paiement_id = " . 1 . ""];
        // $condppiece = "piecereglementclients.paiement_id = " . 1 . "";
        //     $condp2[] = ["Depenses.paiement_id = " . $paiement_id . ""];
        //     $condp3[] = ["Transferecaisses.paiement_id = " . $paiement_id . ""];
        // }
        if ($paiement_id) {
    ...
    public function etatecart()
    {
        ...
        $condp1[] = ["Piecereglementclients.paiement_id = 9"];
        ...
        $paiements = $this->Paiements->find('all')->where(['Paiements.id =9']);

        $this->set(compact('clientData', 'credit_all', 'debit_all', 'paiements'));
    }
    public function impetatecart()
    {
        error_reporting(E_ERROR | E_PARSE);
    ...
    public function impetatcaisse()
    {
        ...
        foreach ($reglements as $reglement) {
            ...
            if ($reglement_id != null) {
                $reglementclient = $this->Reglementclients->find()
                    ->select(['client_id', 'date', 'numeroconca'])
                    ->where(['id' => $reglement_id, $conditions])
                    ->first();

                $lignereg = $this->Lignereglementclients->find()
                    ->select(['Montant', 'bonlivraison_id', 'commande_id', 'factureclient_id'])
                    ->where(['Lignereglementclients.reglementclient_id' => $reglement_id])
                    ->first();
    ...
    public function impetatjournalier()
    {
        error_reporting(E_ERROR | E_PARSE);
        ...
        foreach ($reglements as $reglement) {
            ...
            if (isset($client->Raison_Sociale) && $reglementclient && $reglementclient->date) {
                ...
                $clientData[] = [
                    ...
                    'credit' => '',
                    'num' => $numpiece, // $reglementclient->numeroconca,
                    'type' => 'Réglement',
                    'index' => '2',
                ];
            }
    ...
    public function getblocclients($id = null)
    {
        $id = $this->request->getQuery('idfam');
        ...
        $adresses = $this->fetchTable('Adresselivraisonclients')->find('all', [
            ...
            ->where(["Adresselivraisonclients.client_id = " . $id . ""])->first();
        $adresse = $adresses->adresse;
