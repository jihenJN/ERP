...
if($artic==2){
    $condtype = "";
     ...
     $condtype = "Articles.famille_id  != 1";
}
        $query = $this->Articles->find('all')->where([$cond1, $cond2, $cond3,$condtype]);
        //debug($query);
                ...
                if (isset($this->request->getData('data')['articlefr']) && (!empty($this->request->getData('data')['articlefr']))) {
                    foreach ($this->request->getData('data')['articlefr'] as $j => $p) {
                        ...
                        if ($p['supfr'] != 1) {
                            ...
                            $data['prix'] = $p['prix'];
                            $data['code'] = $p['code'];
                            $data['fournisseur_id'] = $p['fournisseur_id'];
...
foreach($articlesss as $a)
{
   // debug($a->id.' '.$a->Dsignation);
    $articles[$a->id]=$a->Code.' '.$a->Dsignation;
        ...
        $this->set(compact('articlefournisseurs','frs','articles','devices','dat','charges','famillerotations', 'tab', 'articlees','objectifrepresentants', 'commercials', 'mois', 'unitearticles', 'sousfamille2s', 'unites', 'tpe', 'seuil', 'codeart', 'article', 'familles', 'sousfamille1s', 'typearticles', 'codepays', 'codeproducteur', 'val', 'tvas', 'fodec'));
    }
...
//debug($articles);die;
        $this->set(compact('articles','frs','devices','charges','famillerotations', 'commercials', 'mois', 'unitearticles', 'tpe', 'unites', 'article', 'familles', 'sousfamille2s','sousfamille1s', 'typearticles', 'codepays', 'codeproducteur', 'val', 'tvas', 'fodec'));
    }
