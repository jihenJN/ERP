...
class ClientsController extends AppController {
    ...
    public function add() {
        ...
        $this->set(compact('client', 'commercials', 'gouvernorats', 'typeexonerations', 'banques', 'paiements', 'typeclients', 'pointdeventes', 'bureaupostes'));
    }
    ...
    public function edit($id = null) {
        ...
        $commercials = $this->Clients->Commercials->find('list', ['limit' => 200])->all();
        $gouvernorats = $this->Clients->Gouvernorats->find('list', ['keyfield' => 'id', 'valueField' => 'Description'])->all();
        $this->set(compact('bureaupostes', 'pointdeventes', 'typeclients', 'client', 'commercials', 'gouvernorats', 'typeexonerations', 'paiements', 'adressees', 'responsable', 'banquess', 'banques'));
    }
    ...
    public function getbureaupostes($id = null) {
        ...
        $query = $this->fetchTable('Bureaupostes')->find();
        $query->where(['gouvernorat_id' => $id]);
        // debug($query);
        $select = "

        <label class='control-label' for='sousfamille1-id'>Code postal</label>
        <select name='Cp' id='sous' class='form-control select2'  >
					<option value=''  selected='selected' disabled>Veuillez choisir</option>";
        foreach ($query as $q) {
            //  debug($q); 
            $select = $select . "	<option value ='" . $q['codepostal'] . "'";
            $select = $select . " >" . $q['codepostal'] . "</option>";
        }
