...
class ReglementclientsController extends AppController
{
    ...
    public function index($type = null)
    {
        ...
        $factures = $this->Reglementclients->find('all')->contain(['Clients'])->where($condcommercial)->order(['Reglementclients.id' => 'DESC']);

        $bonlivraisons = $this->Reglementclients->find('all')->contain(['Clients'])->where($condcommercial)->order(['Reglementclients.id' => 'DESC']);
    ...
    public function view($type = null, $id = null)
    {
        ...
        foreach ($lignesreg as $l => $li) {
            ...
            if ($li['factureclient_id'] != 0 && $li['factureclient_id'] !=null) {
                $l .= ',' . $li['factureclient_id'];
            } else if ($li['bonlivraison_id'] != 0 && $li['bonlivraison_id'] !=null) {
        ...
        $this->set(compact('caisses','timbre', 'banques', 'type', 's', 'id', 'mtfact', 'mtbon', 'facreg', 'piecereglementclients', 'lignesreg', 'valeurs', 'carnetcheques', 'paiements', 'cli', 'livraisons', 'factures', 'reglement', 'clients'));
    }
     ...
     */
    public function add($type = null, $client_id = null ,$piece)
    {
        $this->loadModel('Factureclients');
        $this->loadModel('Bonlivraisons');
        $this->loadModel('Lignereglementclients');
    ...
    public function edit($type = null, $id = null)
    {
        ...
        if ($cli != null) {
            ...
            $factures = $this->Factureclients->find('all')->where(['Factureclients.client_id =' . $cli]);
            // $factures= $connection->execute("select * from factureclients where (factureclients.client_id=".$cli." and factureclients.totalttc > factureclients.Montant_Regler) OR (factureclients.id in".$l.");")->fetchAll('assoc');
