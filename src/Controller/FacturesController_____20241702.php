...
class FacturesController extends AppController
{
    ...
    public function addindirect($tab = null)
    {
        ...
        $lignelivraisons = $lignelivraisonsTable->find()
            ->select([
                'article_id', 'prix', 'remise', 'fodec', 'tva', 'ttc',
    ...
    public function index($typef = null, $typefact = null)
    {
        ...
        $query = $this->Factures->find('all')->where([$condtype0, $condtype, $cond1, $cond2, $cond3, $cond4, $cond5, $cond6])
        ...
        $this->set(compact('timbre','paiements','validationfactureachat','typef', 'typefact',  'confaieurs', 'factures', 'fournisseurs', 'chauffeurs', 'materieltransports', 'pointdeventes', 'depots', 'cartecarburants'));
    }
    ...
    public function add($typef = null, $typefact = null)
    {
        ...
        $articles = $this->Articles->find('list', ['keyfield' => 'id', 'valueField' => 'Dsignation'])
            ->where([$cond]);
        $this->set(compact('mm', 'conffaieurs', 'chauffeurs', 'articles', 'facture', 'numero', 'livraisons', 'fournisseurs', 'pointdeventes', 'depots', 'cartecarburants', 'materieltransports', 'typef'));
    }
            ...
            if ($this->Factures->save($facture)) {
                ...
                if (isset($this->request->getData('data')['tabligne3']) && (!empty($this->request->getData('data')['tabligne3']))) {
                    ...
                    foreach ($this->request->getData('data')['tabligne3'] as $i => $liv) {
                        ...
                        $this->fetchTable('Lignefactures')->save($lignefactures);
