...
<script>
    ...
    $('.btnfac6').on('click', function() {
        ...
        $.ajax({
            method: "GET",
            url: "<?= $this->Url->build(['controller' => 'Reglementclients', 'action' => 'getidstr']) ?>",
            dataType: "json",
            data: {
                tabValues: tabValues
            },
            success: function(data) {
                alert('Bordereau Versement Traites Ajoutées avec succès');
                window.location.href = wr+'bordereauversementcheques/index/1';
            },
