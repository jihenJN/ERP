...
foreach ($commandeclient->lignecommandeclients as $lignecommandeclient){
        if ($lignecommandeclient->type == 1) {
            ...
            $additionalRows1 = ceil($designationLength / 48); 
            $descriptionLength = strlen($lignecommandeclient['description'] ?? '');
            $additionalRows = ceil($descriptionLength / 48);  // 50 chars per line estimate
            ...
            <td colspan="2" style="border: none; vertical-align: top;padding-left: 0 !important;">
                ...
                <div class="boxbordergray" style="height: 35px;width:99% !important;  font-size: 12px;">
                    <div style="margin-left: 1%;">
                        ...
                        //ucfirst(int2str(sprintf("%01." . $virg . "f", str_replace(",", ".", $commandeclient->totalttc)))); ?>
                    </div>
                </div>
...
//echo chifre_en_lettre11('57663.80',3,'');

function chifre_en_lettre($montant, $devise1, $devise2 = '')
{
    // Assign currency names based on `devise1`
    $dev1 = match ($devise1) {
        1 => 'Dinars',
        2 => 'Dollars',
        3 => 'Euro',
    ...
    // Split the integer and decimal part using string functions
    $parts = explode('.', number_format($montant, 2, '.', ''));
    $valeur_entiere = intval($parts[0]); // Integer part
    $valeur_decimal = intval($parts[1]); // Decimal part as an integer
    ...
    $milliers = intval($valeur_entiere / 1000);
    $reste = $valeur_entiere % 1000;
    $result = '';

    if ($milliers > 0) {
        $result .= convertir_centaines($milliers, $chif, $dizaines) . ' Mille';
    }
    if ($reste > 0) {
        $result .= ' ' . convertir_centaines($reste, $chif, $dizaines);
    }

    // Handle decimal part
    if ($valeur_decimal > 0) {
        $result .= ' et ' . convertir_centaines($valeur_decimal, $chif, $dizaines) . ' ' . $dev2;
    } else {
        $result .= ' et zéro ' . $dev2;
    }
...
// Example usage
//echo chifre_en_lettre('57663.945', 3);
