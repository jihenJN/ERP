    ...
    <tbody>
        ...
        foreach ($commandeclient->lignecommandeclients as  $lignecommandeclient) {
            ...
            if ($lignecommandeclient->type == 1) {
                ...
                $vmp111 = calculateTdHeight($chaine, 16, 1.5, 62);
                
           
                $vmpp122 = calculateTdHeight($lignecommandeclient['description'], 16, 1.5, 62);
                if ($vmp111 <= 24) {
                    
                    if($lignecommandeclient['article_id']){
                    $chaine = $articl[0]['Refggb'];
                    }
                    $vmp111 = calculateTdHeight($chaine, 16, 1.5, 14);
                }
               
               $heiii1 += $vmp111;
                $heiii1 += $vmpp122;
                if ($vmp111==0 && $vmpp122==0) {
                    $heiii1 +=24;
                }

            //     debug($vmp111);
            //    debug($vmpp122);
            //    debug($lignecommandeclient['id']);
              $nbtr++;
                if (($heiii1 >= 400 && $nbpage == 1)|| ($heiii1 >= 700 && $nbpage != 0) ) {
              //  debug($heiii1);
       ...
       // $nbpage += ceil(($indexcc - $ki) / 40);
        ...
        foreach ($commandeclient->lignecommandeclients as $lignecommandeclient):
         ...
         //  var_dump($tabheight);die;
                ...
                <tr class="tr">
                    <td align="center"
                        style="<?php $heitd0=''; if ($heitab>$tabheight[0]['height'] && $pag==0 && $nbpage!=1){ $heitd0=450/(float)$tabheight[0]['nbtr'];} else if ($heipin>$tabheight[$pag]['height'] && $pag!=$nbpage && $nbpage!=1 ) {
                        if ($tabheight[$pag]['nbtr']) {
                        ...
                        class="<?= (($k === $ki) || ($k === 40) || ($k === 38) || ((float)$tabheight[$pag]['nbtr']==$hhhhh) ) ? 'solid-border2' : 'dotted-border' ?>">
...
//var_dump($tabheight);
                ...
                if (($hei >= 400 && $pag == 0)|| ($hei >= 700 && $pag != 0) ) {
