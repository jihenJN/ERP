          ...
          <table id="example1" class="table table-bordered table-striped">
            <thead>
              <tr>
                ...
                <th width="10%" align="center"><?= ('Fréquence') ?></th>
                <th width="10%" align="center"><?= ('Montant Crédit') ?></th>
                <th width="10%" align="center"><?= ('Taux d\'intérêt') ?></th>
                <th width="10%" align="center"><?= ('Montant Remboursement') ?></th>
                <th width="10%" class="actions text-center"><?= __('Actions') ?></th>
              </tr>
            </thead>
            <tbody>
            <?php foreach ($typecredits as $i => $type): ?>
                <tr>
                <td><?= h($type->name) ?>
                    <?php echo $this->Form->control('id', ['index' => $i, 'id' => 'id'.$i, 'value' => $type->id, 'label' => '', 'type'=>'hidden','champ' => 'id', 'class' => 'form-control']); ?>

                </td>
                <td><?= h($type->frequence->name) ?>
                <td><?= h($type->montantcredit) ?>

                </td>
                <td><?= h($type->taux) ?>

                </td>
                <td><?= h($type->montantremb) ?>
...
$('.verifiercmd').on('click', function() {
 // alert('hello');
  ...
  $.ajax({  
    method: "GET",
    url: "<?= $this->Url->build(['controller' => 'Lignecomptes', 'action' => 'verif']) ?>",
    dataType: "json",
    data: {
      id: id,
    },
   ...
   //  alert(data.Ticketventes);
      if (data.Types != 0) {
        ...
        if (confirm('Voulez-vous supprimer cet enregistrement')) {
            alert('ok supp');
          document.location = wr+"Typecredits/delete/" + id;
        }
      }
    }
  })
