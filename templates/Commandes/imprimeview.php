...
// $lignecommandesArray = $lignecommandes->toArray();

// $maxLinesPerPage = 22;
// $totalLines = count($lignecommandesArray);
// // var_dump($totalLines);
...
$filteredLines = array_filter($lignecommandesArray, function ($line) {
    return $line['qte'] != 0;
});
                        ...
                        <tr>
                            ...
                            <td align="center" style="width: 30%;border: none;height:4% !important;">
                                <b style="font-size: 18px!important; display: block; margin-bottom: 0.5em;">
                                    <?php echo 'BON COMMANDE'; ?>
                                </b>
                                <b style="font-size: 16px!important; font-weight: normal; display: block; margin-bottom: 0.5em;">
                                    <?php echo 'N° : ' . '' . $commande->numero; ?>
                                </b>
                                <b style="font-size: 16px!important; font-weight: normal; display: block; margin-bottom: 0.5em;">
                                    <?php echo '' . $this->Time->format($commande->date, 'dd/MM/y'); ?>
                                </b>
                            </td>
                            <?php
                            ...
                            if (isset($commande->client)) {
                                ...
                                if ($commande->client_id == 12) {
                                    $nom_prenom = h($commande->nomprenom);
                                    ...
                                    $raison_sociale = h($commande->client->Raison_Sociale);
                            ...
                            <td class="client-info" style="width: 35%; border: none; font-size: 15px; height: 70px; vertical-align: top;">
                                ...
                                <b style="font-size: 16px; font-weight: normal; display: block; margin-bottom: 0.5em;">
                                    &nbsp;<?php if (isset($commande->client)) {
                                                if ($commande->client_id != 12) {
                                                    echo h($commande->client->Adresse);
                                                } else {
                                ...
                                <b style="font-size: 16px; font-weight: normal; display: block; margin-bottom: 0.5em;">
                                    &nbsp;<?php
                                            if ($commande->client_id != 12) {
                                                echo 'M.F : ';
                                                if (!empty($commande->client)) {
                                                    echo h($commande->client->Matricule_Fiscale);
                                                }
                                            } else {
                                                echo 'I.D : ';
                                                if (!empty($commande->client)) {
            ...
            <div style="margin-top: -17.6%;">
                <table style="border-width: 0px;  width: 100%; border-collapse: collapse; margin-top:31%;">
                    ...
                    <tbody>
                        ...
                        for ($i = $start; $i < $end; $i++) :
                            $pp++;
                            $lignecommande = $lignecommandesArray[$i];
                            ///  debug( $lignecommande);
                            ...
                            <tr class="fixed-height-row">
                                <td style="text-align:left;width:50%;">
                                    <?php if (!empty($commande->observation)) : ?>
                                        <b><?php echo $commande->observation; ?></b>
                                    <?php else : ?>
                                        <b>&nbsp;</b>
                                    <?php endif; ?>
                                </td>
                        ...
                        ?>
                            ...
                            <!-- <tr>
                                <td colspan="6" style="text-align:left;">
                                    <b><?php //echo $commande->observation; 
                                        ?></b>
                                </td>
                            </tr> -->
                        <?php   }  ?>
            ...
            ?>
                <div style="display:flex; margin-left:8%;">
                    ...
                    <table style="width: 60%;border-collapse: collapse;border-radius: 15px;margin-top:6%;">
                        ...
                        <tbody>
                            <?php $lignestable = TableRegistry::getTableLocator()->get('Lignecommandes');
                            ...
                            $query->select([
                                ...
                                ->group('Lignecommandes.tva');

                            // Execute the query
                            $results = $query->toArray();
                            $fodquery = $lignestable->find();
                            $fodquery->select([
                                'fodec' => 'Lignecommandes.fodec',
                                'base' => $query->func()->sum('qte*ml*prix - (qte*ml*prix)* (remise / 100)'),
                                'total' => $query->func()->sum('(qte*ml*prix - (qte*ml*prix)* (remise / 100)) *ifnull(fodec,0) / 100')
                            ])
                                ->where(['Lignecommandes.commande_id' => $commande->id])
                                ->group('Lignecommandes.fodec');
                    ...
                    <table style="width: 55%; border-collapse: collapse; border-radius: 15px; margin-top: 2.5%; margin-left: 40%!important;">
                        <thead>
                        </thead>
                        <tbody>
                            <tr>
                                <td style="text-align: left; width: 50%; vertical-align: top;">
                                    ...
                                    <b style=" width: 100px;margin-left:-10!important; line-height: 1.5;"> <?php if ($commande->remise != 0) { ?><?php echo 'Rem Mt:'; ?> <?php } ?><br></b>
                                ...
                                <td style="text-align: left; width: 50%; vertical-align: top;">
                                    ...
                                    <b style="display: inline-block; width: 130px; text-align: right; line-height: 1.5;">
                                        <?php if ($commande->remise != 0) { ?> <?php echo number_format(abs($commande->remise), 2, ',', ' '); ?> <?php } ?><br>
                                    </b>

                                    <b style="display: inline-block; width: 130px; text-align: right; line-height: 1.5;">
                                        <?php echo number_format(abs($commande->total), 2, ',', ' '); ?><br>
                                    </b>
                                    <b style="display: inline-block; width: 130px; text-align: right; line-height: 1.9;">
                                        <?php echo number_format(abs($commande->tva), 2, ',', ' '); ?><br>
                                    </b>
                                    <!-- <b style="display: inline-block; width: 130px; text-align: right; line-height: 1.5;">
                                        <?php //echo $timbre; 
                                        ?><br>
                                    ...
                                    <b style="font-size: 16px!important; margin-top: -6.6px!important; font-weight: bold; display: inline-block; width: 130px; text-align: right;">
                                        <br><?php echo number_format(abs($commande->totalttc), 3, ',', ' '); ?>
                                    </b>
                                </td>
                            </tr>
                        </tbody>
                ...
                <table style="width: 65%;border-collapse: collapse;border-radius: 15px;margin-top:-6%;margin-left:-3%;">
                    <thead>
                    </thead>
                    <tbody>
                        <?php
                        ...
                        <tr class="tr">
                            <td align="center" height="20px" width="40px">
                                <b style="color:#A8A8A8 ;margin-left:13% ;margin-top:-6%!important; font-size: 15.5px;!important "><?php echo  'ARRETE LA PRESENTE COMMANDE A LA SOMME DE :';
                                                                                                                                    ?></b>
                            </td>
                        </tr>
                        <tr class="tr">
                            ...
                            <td align="center" height="20px">
                                <b style="font-weight: normal;margin-top:-5% !important;font-size: 18px;!important"> <?php echo int2str($commande->totalttc, 1, 1) ?></b>
                            </td>
